				...
				if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`[❗] Falló, al momento de convertir la imagen a sticker`)
							})
							.on('end', function () {
								console.log('Finish')
							        exec(`webpmux -set exif ${addMetadata(pack, author)} ${ran} -o ${ran}`, async (error) => {
...
break
   case 'wsrd hwrtu wrt uwruj wruj ':
if (!isGroup) return reply(mess.only.group)
if (!isGroupAdmins) return reply(mess.only.admin)
		...
		if (userLevel === undefined && userXp === undefined) return reply(mess.levelnol)
            ...
            break
                                /*case 'nsfwtrap':
										...
										case 'randomhentaio': 
						try {
							if (!isNsfw) return reply('❌ *fds gedh w Desactivado* ❌')
					...
					case 'wrth wrt ': 
						try {
							if (!isNsfw) return reply('❌ *werth wrth  Desactivado* ❌')
					...
					case 'wrth rwth w':
						try {
							if (!isNsfw) return reply('❌ *wrth wert h Desactivado* ❌')
					...
					case 'wryh wetywe t':
						try {
							if (!isNsfw) return reply('❌ *wery qetu  Desactivado* ❌')
					...
					break*/
				case 'sfksrky swyk isr':
						try {
							if (!isNsfw) return reply('❌ *sr tksrtk  Desactivado* ❌')
					...
					case 'srtk etkek ':
						try {
							if (!isNsfw) return reply('❌ *estj wrj Desactivado* ❌')
					    ...
					    break
					case 'wrki rw kietyk o':
						try {
							if (!isNsfw) return reply('❌ *wrti rt i Desactivado* ❌')
						...
						break
					case 'opyrulyriññ yñir ':
						try {
							if (!isNsfw) return reply('❌ *ñiu ñ Desactivado* ❌')
						...
						break
					case 'uiñ utiñ utiñ tyi':
						try {
							if (!isNsfw) return reply('❌ *tyil ut Desactivado* ❌')
						...
						break
					case 'tuipñuiñ tyiñtui':
						try {
							if (!isNsfw) return reply('❌ *utiñltyiñ  Desactivado* ❌')
						...
						break
						case 'utiñ7r 8tñr7ñr r':
						try {
							if (!isNsfw) return reply('❌ *ryu irturt Desactivado* ❌')
					...
					case 'tipsBot':
						when = budy.slice(1)
						if (args.length < 1) return reply(mess.only.userB)
						const tipsBot =['Alguien dijo community day de deino?\nConsulta cúal será el siguiente CD en @seguimiento#cd','Hora destacada de Rufflet?\nCuándo? Dónde?\n\n¡Descubre de quién será la próxima hora destacada, los bonus, los IV recomendados, horarios y coordenadas con @seguimiento#hd','Smeargle lvl 50 :v\n\nUsa @pokemon#maxear# "1 o 2" para saber que pokemon son mejores para maxear','Nido de arceus ???\nConsulta los nidos en @nidos y si solo quieres un Pokémon usa el comando @nidos  más # y el Pokémon por ejemplo @nidos#eevee','Te salen mal las misiones?\nEn plan, misiones que no son?\n\nConsulta con el comando @mision#fail para ver el problema','Lugares de farmeo?\nDónde?\n\nEscribe @lugares#farmeo y tendrás los mejores lugares del mundo para farmear como si no hubiera un mañana','No sabes cómo vencer a los líderes del Team Go Rocket o al jefe Giovanni?\n\nEscribe @+El miembro del TGR que desees vencer, ejemplo @cliff','Miedo de ser expulsado del grupo ????\n\nConsulta las reglas del grupo en @reglas','Qué actividades habrán en este mes?\n\nDescúbrelo con el comando @eventos#+el mes, ejemplo @eventos#noviembre','Siempre espera unos minutos más cuando el contador lleve a cero, no vaya a ser que por un error pierdas ese preciado shiny','Un poco sad ??\nUna de nuestras frases te motivará en @frase#motivadora','Recuerda que el compañero puede hacer ciertas actividades que tienen registro, evitalo y no pongas al compañero Pokémon en el mapa (no darle Baya)','Recuerda que hay un límite de capturas y de pokestop giradas\nSi superas ese límite, tendrás un leve baneo','Pokémon malos para pvp ??\nConsulta @pvp#1500 o @pvp#2500 y verás parte de los mejores pokemon para pvp','Viajemos a la velocidad de la luz \nConsulta @info#softban y consulta los tiempos y acciones para evitar el baneo','Miedo al éxito???\nConsulta @fly#help y tendras un pequeño tutorial de cómo ser fly *(solo android)*','Regionales ?? \n\nEn @regionales#mundial consulta las coordenadas de los regionales como\nMr mime\nKangashkan\nHeracross, etc','Arceus LVL 50 IV100 *UwU\n\nUsa @legendarios#maxear y verás las recomendaciones de qué legendarios deberías de maxear','Rank Leyenda 💪\nUsa @temp#gbl y obtén la información de la temporada actual de la Go Battle League *(Go Battle LAG)*','Si usas apps de terceros, está penalizado por Niantic, asegúrate de usarlos con moderación y prepárate por si algún día te banean la cuenta']
						const ple_2 = tipsBot[Math.floor(Math.random() * tipsBot.length)]
						pod = await getBuffer(`https://media1.giphy.com/media/428dIJljoEbxS/giphy.gif?cid=ecf05e4713naofw1um6vyku9yum0sqkzd1rgehwww1tgmbjs&rid=giphy.gif&ct=g`)
						client.sendMessage(from, pod, image, { quoted: mek, caption: '*Minuto Tip*\n\n'+ ple_2 })
					break
                default:
